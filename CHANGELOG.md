...
- 升级方式调整为运行安装器后自动识别版本、保留数据和配置，设置页改为“版本与维护状态”；移除手填目标版本和旧页面准备维护的正常流程。
- 增加本机管理员维护请求和旧版兼容路径；已知活动拒绝，离线/未知时要求现场停机确认，未完成档案和未知命令继续保留。
- 改善程序暂存、按卷空间预检、同版本修复和失败事务恢复；数据库、账户、密钥和自定义路径保留，首次正式交付不自动删除恢复备份。
- 增加安全的健康检查诊断与 UTC 日志，记录具体检查、版本、状态与空间，排除秘密材料。
- 软件正式版与指定设备组合资格分开。稳定发布门禁绑定真实 Windows 安装升级、SmdBench、当前 CI 提交和实际包/日志摘要；缺少证据则拒绝。

本节描述本版本的软件变更；是否已验收和发布以[任务清单](tasks/todo.md)及实际 Release 所附机器证据为准。Win10/11 现场桌面、目标固件、物理联锁及国标符合性仍未验收，详见[发布证据范围](docs/release-acceptance.md)。旧 RC 标签和资产保持不变。
...
- 修正 PDF 温度差减号及页尾分隔符缺字、结果标题与表格跨页分离的问题，保留计算数值及其他导出格式。
- 服务停止时限制HTTP/WebSocket请求排空，保留未知操作恢复；清理等待正在停止的服务，失败证据仅收集固定退出阶段。
