...
def _standard_stages() -> list[dict]:
    ...
    for name, target, rate, exit_signal, exit_value, n2, co in [
        ...
        stages.append(dict(name=name, kind='ramp', furnace_target_c=target, ramp_c_min=rate,
                           n2_l_min=n2, co_l_min=co, exit=dict(signal=exit_signal, value=exit_value), timeout_s=21600))
    stages.extend([
        dict(name='料层达到1580℃后保持30分钟', kind='hold', furnace_target_c=1600,
             n2_l_min=3.5, co_l_min=1.5, exit=dict(signal='elapsed_s', value=1800), timeout_s=3600),
        dict(name='氮气置换并冷却', kind='cool', n2_l_min=2, co_l_min=0,
             exit=dict(signal='burden_c', comparison='lt', value=200), timeout_s=86400),
    ])
    return stages
...
def standard_template() -> RecipeDefinition:
    return RecipeDefinition(name='GB/T 34211-2017 标准候选模板', mode='standard',
...
def validate_for_device(recipe: RecipeDefinition, profile: dict | None, capabilities: list[str]) -> dict:
    ...
    try:
        ...
        return {'executable': False, 'errors': errors + ['device_safety_profile_missing_or_invalid'],
    ...
    for i, stage in enumerate(recipe.stages):
        prefix = f'stage_{i + 1}:'
        for value, maximum, code in [(stage.furnace_target_c, limits.temperature_max_c, 'temperature_max'),
    ...
    if (final.kind != 'cool' or final.co_l_min != 0 or final.n2_l_min <= 0 or final.exit.signal != 'burden_c'
            or final.exit.comparison != 'lt' or final.exit.value > limits.safe_end_burden_c):
        errors.append('safe_cooling_end_required')
    return {'executable': not errors, 'errors': errors, 'deviations': recipe.deviations(),
            'safety_profile_version': limits.version, 'rules_reference': limits.rules_reference}
