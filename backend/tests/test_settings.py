...
async def test_create_and_duplicate(db_session):
    ...
    with pytest.raises(HTTPException) as ei:
        await users_route.create_user(
            users_route.CreateUserRequest(username="alice", password="x", role="observer"), db_session
        )
...
async def test_self_lockout_blocked(db_session):
    ...
    res = await users_route.create_user(
        users_route.CreateUserRequest(username="root", password="pw", role="admin"), db_session
    )
