"""Missing old application tickets do not prevent installer-owned maintenance."""
...
def files(root):
    return {
        ...
        if path.is_file() and ".staging" not in path.relative_to(root).parts
    }
...
def installed(tmp_path, monkeypatch):
    ...
    old_program = install / "versions/0.3.0-rc.5/SmdService/SmdService.exe"
    ...
    monkeypatch.setattr(
        sys, "argv", ["SmdUpdate.exe", "--non-interactive", "--install", str(install), "--package", str(package)]
    )

    def unknown(*args, **kwargs):
        raise updater.ConfirmationRequired("现场状态未知")

    monkeypatch.setattr(updater, "authorize", unknown)
...
def test_unprepared_upgrade_identifies_target_and_preserves_existing_installation(installed, gate):
    ...
    assert "无需登录旧版或填写版本号" in str(failure.value)
    assert files(installed.data) == before_data
    assert files(installed.install) == before_install
    assert not installed.package.exists()  # Unused payload is disposable; live data/program are not.
    assert installed.platform.mock_calls == [call.release_backend_guard()]
