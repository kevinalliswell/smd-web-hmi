...
维护角色：后端负责人、STM32 固件负责人。基线 `dev@855c84d`，2026-09-05。这里的“当前约定”指仓库客户端和 Mock，尚未证明与真实固件一致。原 V0.1 需求全文已[归档](archive/dev-855c84d/docs/GB_T34211_HostComm上位机通信协议开发需求说明.md)；供应商冻结表、数据字典、SOP 等仍见[外部依赖清单](待确认事项与接口对齐清单.md)。
...
```json
{
  ...
  "payload": {"client_name": "smd-web-backend", "client_version": "0.3.0-rc.1"}
}
```

`hello_ack.payload` 的现有约定包含 result、fw_version、hw_version、device_profile_version、parameter_crc 和 capabilities。目标要求在握手结果被接受、版本兼容后才建立可用会话；能力声明必须满足所请求操作。基线客户端尚未完整校验 rejected/不兼容版本，不应据“TCP 已建立”判定设备可控制。
...
| get_status / status_snapshot | 按下一条同类型快照满足等待者 | 冻结请求关联字段；推送与响应不可混淆 |
| command / command_result | 按 payload.request_msg_id 关联 | 固定报文 ID、逻辑操作 ID、结果查询和板端去重语义 |
| get_parameters / parameters_snapshot | 按类型等待回读 | 回读须来自匹配请求并核对参数及版本 |
| event | 推送事件，报警落库 | 事件码、序号、重连活跃报警和缺口恢复 |
| log_request / log_chunk / log_result | 需求存在，基线未接入 | Q7 冻结格式、序号、校验和补传 |

`command_result=accepted` 只能表示设备受理；实际完成由明确终态或可关联结果确认。结果丢失时记录 unknown，不能把 HTTP 超时写成“设备未执行”。基线重试会产生新 msg_id，需修复；去重窗口、跨重启行为未获验证前禁用有副作用的自动重发。

诊断命令维持只读 get_status/get_parameters。所有控制请求走认证、权限、后台状态、二次确认（适用时）和操作审计。

## 4. 状态、时间、质量与队列

规范状态位置为 `payload.state_machine.current_state`；基线还存在读取 `system.current_state` 的路径，属待修复问题。前端阶段别名只决定显示，不能放行操作。完整枚举、End 与冷却完成的区别、故障复位条件及实验 ID 生命周期由 Q1 冻结。
...
目标使用有界处理队列；遥测允许按明确策略合并最新显示值，但原始采样丢失必须标记。报警/事件与命令回执走独立优先路径；超载应暴露容量、队列年龄、丢弃计数和持久化错误，不能静默丢弃后报告正常。

## 5. 参数、配方与兼容

基线参数分组为 process、mfc、temp_program、ai_calib；字段值见 Mock，真实量程和含义以 Q5 冻结表为准。尤其不能把工艺 N2 5 L/min 保护与结束后 N2 2 L/min 置换混为一个默认值。

当前上位机 CRC 使用排序 JSON 的 CRC32；它不证明供应商使用相同字节序列。Q4 必须给出位宽、多项式、初值、反射、最终异或、字符编码、浮点规范化、覆盖范围及双方黄金向量。回读的设备 CRC、值和参数版本均需明确核对。
