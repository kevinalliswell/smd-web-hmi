...
> 适用范围：本文记录 `0.3.0-rc.2` 的 HostComm **1.0** 及当时的候选要求。用户已确认固件尚未开发，并授权项目设计新协议；新固件开发入口为 [HostComm v2.0](hostcomm/v2/README.md)，迁移任务见[固件与上位机实施路线](hostcomm/v2/firmware-plan.md)。下文不构成已存在固件或 v2.0 已被当前安装包支持的证据。
