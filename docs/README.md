...
2. 用户已确认本项目固件尚未开发、MCU 为 STM32H750，并授权项目设计通信契约。完整器件后缀、板卡修订、PHY 与外部存储仍需工程资料。HostComm v2.0 是新固件与上位机适配的共同设计依据；0.3.0-rc.2 仍使用 1.0，不能直接接入 v2.0。设计、双方实现一致性和真机验收分别记录；无需等待不存在的既有供应商协议才能开展开发。
