...
维护角色：项目维护者。[v0.3.0 上位机软件正式版](https://github.com/kevinalliswell/smd-web-hmi/releases/tag/v0.3.0)已于 **2026-09-15 09:49:48 UTC** 发布，提供保留账户、配置、配对密钥及实际数据库路径的覆盖安装，无需手填目标版本。发布提交为 `404ecb50edf23864ed482e68cc4a1933deb3383f`，软件与设备验收边界见[ADR-011](decisions/ADR-011-overwrite-install-and-software-release.md)。

主线和标签分别完成必要检查、真实 rc.4/rc.5 升级、完整 TLS 安装版实验与报告、故障恢复及资产门禁。标签工作流八项任务全部通过，Release 非草稿、非预发布，包含21项发行资产，实际下载和校验值均已核验，工具ZIP内部清单也已核验。构建身份、历史失败与分层证据见[本轮验证记录](verification/2026-09-13-overwrite-install.md#标签构建与正式发布)，任务见[路线图](../tasks/plan.md)和[待办](../tasks/todo.md)。实际发行字节与范围以该 Release 的机器验收和 `SHA256SUMS.txt` 为准；本轮19项联调结果与自动门禁的覆盖范围分别记录。

仓库保持私有，GitHub Release 发行资产的下载需要本仓库访问权限。

上一候选 `0.3.0-rc.5` 已完成 HOST-2006—2009 的软件实现及 Windows CI 安装版联调，完整构建身份、实际产物摘要和清理结果见[安装版闭环验证](verification/2026-09-08-installed-hostcomm-loop.md)。候选交付状态、最终提交与字节以对应标签流水线及 Release 附件为准。当前固件文档修订为 `2.0-doc.4`，协议仍为 `2.0 / 2.0-design.1`；具体实施与联调先读[技术交底](hostcomm/v2/technical-briefing.md)，职责与未验边界见[固件交接](hostcomm/v2/firmware-handoff.md)和[兼容核对](hostcomm/v2/compatibility.md)。Win10/11 WebView2 人工验收、真实 STM32、安全联锁与持续运行验收仍未完成。

2026-09-20 用户确认 `0.3.0` 安装成功，固件工程师已在开发；固件构建材料、配对、真实通信及实验仍未交付验收。离线技术资料包保留完整源码目录、版本及摘要清单，允许无仓库权限的接收者离线阅读；包中不含安装器、联调工具二进制或开发环境，现场凭据另行受控交接。
...
| [STM32H750 技术交底](hostcomm/v2/technical-briefing.md) | 固件工程师怎样准备、配对、实现报文、首次联调并记录验收？ | 后端/固件/硬件/测试负责人 |
...
| [CI 运行器与平台检查](ci-runners.md) | Mac 检查和 Windows 自托管纯编译如何隔离，哪些平台验收必须保留？ | 维护/测试负责人 |
...
2. 用户已授权项目设计通信契约，MCU 为 STM32H750；截至2026-09-20固件开发中、尚未交付验收。补充[板卡图片](hardware/stm32h750vbt6-board.md)已指明 STM32H750VBT6、LAN8720、W25Q128、24C02 与 ADS8688；板卡修订、原理图、复用冲突和实板能力仍需核实。HostComm v2.0 是新固件与上位机适配的共同设计依据；0.3.0-rc.2 仍使用 1.0，不能直接接入 v2.0。设计、双方实现一致性和真机验收分别记录；无需等待不存在的既有供应商协议才能开展开发。
