...
> 适用范围：本文保留 `0.3.0-rc.2` 的 HostComm **1.0** 客户端与 Mock 行为。2026-09-06 用户确认固件尚未开发；新固件按项目设计的 [HostComm v2.0](hostcomm/v2/README.md) 实施。本文中的待冻结字段及供应商假设是旧实现背景，不要求先取得既有固件。v2.0 与本协议不兼容，当前安装包尚未适配。

维护角色：后端与固件负责人。2026-09-06 软件实现约定，版本 `0.3.0-rc.2`。本文描述实际字段，**不是供应商已冻结的固件接口**。决策见 [ADR-006](decisions/ADR-006-candidate-experiment-contracts.md)，基础帧与外部交付见 [HostComm 总览](GB_T34211_HostComm上位机通信协议开发需求说明.md)和[待确认清单](待确认事项与接口对齐清单.md)。
