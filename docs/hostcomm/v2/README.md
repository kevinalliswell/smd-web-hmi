...
文档修订：`2.0-doc.4`，2026-09-20；应用基线为已发布的 **SmdHmi `0.3.0`**，设计基线和线上握手仍为 `2.0-design.1`，协议字段仍为 `protocol_version="2.0"`。本目录是项目拥有的实现契约；STM32H750 固件开发中、尚未交付验收。软件发布、用户安装结果和真实设备组合验收分别记录，不能相互替代。
...
固件工程师先读[技术交底主文档](technical-briefing.md)，按板卡准备、离线配对、消息实现和首次联调顺序工作；[固件开发交接](firmware-handoff.md)维护职责、交付材料与变更规则。[兼容核对](compatibility.md)和[修订记录](revisions.md)说明发布基线与未验边界。本轮只补充文档及离线技术资料，不修改生产代码、Schema 或发行资产。

## 开发入口

| 交付物 | 使用方法 |
|---|---|
| [技术交底与首次联调](technical-briefing.md) | 交底会议、环境回填、26 种消息实现清单和真实板卡验收记录的主入口 |
...
| 层次 | 本次交付状态 | 还需要什么 |
|---|---|---|
| 协议设计 | design.1作为线上基线；doc.4补齐0.3.0技术交底和离线阅读入口 | 线上语义变更须评估设计/协议版本，说明性修订只升级文档号 |
| 可执行参考 | 有消息/配方模型、编码检查和黄金向量 | C 端独立实现跑相同向量；它不是设备模拟器或控制固件 |
| 上位机运行 | 0.3.0已发布；2.0客户端、配对、操作/配方/源日志及安装版模拟器链路有软件证据 | 真实板卡通信、目标板资源和实体安全按FW-09/10验收 |
| STM32 固件 | 开发中、尚未交付验收 | 驱动、实时任务、持久化、TLS 和契约实现 |
...
初始设计检查见[设计验证](../../verification/2026-09-06-hostcomm-v2.md)；历史文档核对见[交接审查记录](../../verification/2026-09-07-firmware-handoff.md)。
