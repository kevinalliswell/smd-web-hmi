...
设计修订：`2.0-design.1`，2026-09-06。协议字段 `protocol_version="2.0"`。本目录是项目拥有的设计基线，可据此开展上位机适配和固件开发；目标固件尚未开发，旧安装包 `0.3.0-rc.2` 仅使用 HostComm 1.0；`0.3.0-rc.3` 增加显式选择的 2.0 上位机适配和无执行器模拟器，见[运行与配对](runtime.md)、[软件验证](../../verification/2026-09-06-hostcomm-v2-runtime.md)。固件与真机验收仍独立进行。
...
| 上位机运行 | rc.3 已实现 2.0 客户端、配对、操作/配方/源日志恢复；保留 1.0 | 对应提交 CI、候选资产与真实固件联调分别验收 |
