...
设计修订：`2.0-design.1`，2026-09-06。协议字段 `protocol_version="2.0"`。本目录是项目拥有的设计基线，可据此开展上位机适配和固件开发；目标固件尚未开发，当前安装包 `0.3.0-rc.2` 使用 HostComm 1.0，尚不能连接这里定义的 2.0 设备。

用户已确认不需要等待既有固件来反推契约。协议语义由本项目定义；用户随后确认 MCU 为 STM32H750；完整料号/板卡修订、外部存储、网口 PHY、仪表驱动、标定和安全配置需在工程资料与实测中补齐。[ADR-008](../../decisions/ADR-008-hostcomm-v2-design.md)记录版本和架构决策。
...
| [STM32H750 板卡配置](stm32h750-profile.md) | 核对已知 MCU、外部存储/PHY 待补项和资源验收 |
...
| 层次 | 本次交付状态 | 还需要什么 |
|---|---|---|
| 协议设计 | 本目录定义 2.0 开发基线 | 后续修改须修订设计版本、Schema、向量和兼容说明 |
| 可执行参考 | 有消息/配方模型、编码检查和黄金向量 | C 端独立实现跑相同向量；它不是设备模拟器或控制固件 |
| 上位机运行 | 仍为 1.0；已有操作/数据/配方等内部服务 | 2.0 适配器、TLS 配对、日志回填和恢复联调 |
...
本轮设计检查与独立审查记录见[设计验证](../../verification/2026-09-06-hostcomm-v2.md)。
