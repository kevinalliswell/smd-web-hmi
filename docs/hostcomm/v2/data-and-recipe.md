...
文档修订：`2.0-doc.4`；设计基线：`2.0-design.1`；协议：`2.0`。本次不改变握手设计版本。[Schema 与固定向量](../../../contracts/hostcomm/v2/README.md)定义字段形状，[状态契约](state-and-recovery.md)定义时序。下列数值倍率是协议编码选择，不是传感器精度声明，也不是设备安全量程。
