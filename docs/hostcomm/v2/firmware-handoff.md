...
文档修订：`2.0-doc.3`，2026-09-08。适配对象为本轮 SmdHmi `0.3.0-rc.5` 候选版本；线上仍使用 `protocol_version="2.0"`、`design_revision="2.0-design.1"`。文档修订号不能填入报文。源码、候选包与文档的版本关系见[修订记录](revisions.md)。

本文件是固件人员的首读入口。已有设计可以开始实现；[兼容核对表](compatibility.md)列明主机实现限制，当前没有可烧录的本项目固件，也没有双方签署的真机兼容结论。
...
| H1 安装版软件联调 | 独立SmdBench连接本轮安装版服务与同机TLS模拟器，通过Chromium操作共用后台 | Windows CI的身份握手、配方、运行/停止/冷却、归档、实际报告及补传通过；清理、封装与下载产物核验通过，结论绑定[本轮验证](../../verification/2026-09-08-installed-hostcomm-loop.md)中的提交和字节；Win10/11 WebView2人工验收另列M4 |
...
H1与H2可并行。目标板早期只实现以太网或少量消息时，用台架诊断验证；不得假报四项能力以骗过主机握手。能通过hello也不代表profile已批准或允许接入危险执行器。
