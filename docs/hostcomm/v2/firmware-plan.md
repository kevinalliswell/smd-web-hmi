...
文档修订：`2.0-doc.4`（2026-09-20）；应用基线为已发布 SmdHmi `0.3.0`，设计基线 `2.0-design.1`、线协议 `2.0` 不变。固件开发中、尚未交付验收；上位机适配、离线配对与模拟器已有软件证据。用户确认0.3.0安装成功仅登记安装结果，不替代真实通信验收；具体开工和联调步骤见[技术交底](technical-briefing.md)。
...
已有交付包括协议设计、机器可读格式、向量与检查器，以及 HOST-2001—2009 列明的上位机软件、模拟器回归和安装版CI验收。纯 C 核心、STM32 固件、双方台架一致性及指定设备实测仍待完成；`implemented` 不表示 FW-09、Windows 现场或 M5 已通过。

近期按 **技术交底与C核心 → 目标板通信和持久事务 → 实装驱动与安全实验 → 指定组合验收** 推进。安装版与同机TLS模拟器的0.3.0证据已在[正式版验证](../../verification/2026-09-13-overwrite-install.md)记录，可用[同机联调入口](runtime.md#7-独立windows联调工具)在隔离环境复现；它不是目标板测试工具。固件团队按交底回填实际构建/工程资料，并用同一固定向量及可注入时钟/存储验证C核心。
