...
认证状态、实时源记录及已验证回补发现未知device/run时建立持久待核查项，原始记录继续保留。历史试验和当前实验页提供入口；读取不触发控制，归属核查不阻挡明确目标的安全停止。

| 接口 | 行为 |
|---|---|
| `GET /api/run-recoveries?limit=50&offset=0` | 分页查看，limit上限100 |
| `GET /api/run-recoveries/{id}` | 身份、来源、边界、缺口、审查版本及回放进度 |
| `POST /api/run-recoveries/{id}/binding` | 默认创建恢复档案；关联已有实验必须有持久启动请求证明同一身份 |
| `POST /api/run-recoveries/{id}/replays` | 审计请求回放，后台分批执行并持久保存进度 |

两个POST仅admin/maintainer允许，请求含`reason`、`idempotency_key`、`expected_review_revision`；binding可选`target_test_id`。版本或幂等内容冲突返回409，同内容重试返回原结果；不能改绑到另一个实验。

真实开始时间`start_time`允许null，`discovered_at`是发现时间；未知模式显示未知，缺失配方/样品/配置不能取当前值替代。回放与投影进度同事务提交，重复源不重复归档，历史回放不更新实时状态。已全局归档报警/事件通过关联查询进入档案，不复制报警或更改原记录。

回放未完成或存在证据冲突时不得生成最终报告。人工补录不能伪造测定、安全边界；ack_run仍需归属核查、已保存运行证据及板端真实安全/报警条件。旧人工关闭入口不得用布尔确认替代v2源边界。
