...
log_request 指定日志身份、起点、明确终点与限额；先通过 status.log.newest_record_seq 取得最高已提交记录再请求，服务端锁定这一 cut，不把传输中新样本追加进已有响应范围。公布可读最早/最新位置、实际选择范围及缺失区间。每次最多 10000 条、16 MiB，超过请求限额时拒绝并要求缩小范围，不能无标记截断；最多 32 个有序不重叠缺口，更多则返回 range_unavailable 并要求拆分请求。未知日志身份或无法提供可信 highwater 时返回 error，不能制造一个高水位来填充 log_result。越界、已淘汰、掉电丢失和介质损坏必须明确返回，不能伪造空的“完整成功”。
