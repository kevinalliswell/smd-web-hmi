...
文档修订 `2.0-doc.4`；设计基线 `2.0-design.1`；线上 `protocol_version` 仍为 `2.0`，握手 `design_revision` 仍为 `2.0-design.1`。本次补充0.3.0技术交底说明，不修改生产代码或线协议字段。交接入口见 [README](README.md) 和[固件交接资料](firmware-handoff.md)。
...
上位机在发送每批 `log_request` 前执行本地回调准入：最多等待 3 秒、最多容纳 4 个等待者，只等待进入准入时已经接收的回调（包含正在落盘的一项），后到消息不延长该批等待。准入不占线上请求槽；回调队列满或准入超时时不发送本次日志请求，连接、session 或 boot 改变则使准入失效。该机制用于避免本地持久化积压消耗尚未开始的日志传输预算，不改变上述线上总预算、3 秒无进展时限、8 秒租约或任何报文字段；协议和握手仍为 `2.0 / 2.0-design.1`，当前说明修订为 `2.0-doc.4`。
...
以上由[ADR-010](../../decisions/ADR-010-installed-hostcomm-loop.md)明确。rc.4旧行为及当时缺口保留在doc.2历史提交；HOST-2007/2008既有实现与测试见[验证记录](../../verification/2026-09-08-installed-hostcomm-loop.md)。固件仍独立验证8秒租约、持久化和实际安全动作。
