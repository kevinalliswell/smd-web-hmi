...
上位机在发送每批 `log_request` 前执行本地回调准入：最多等待 3 秒、最多容纳 4 个等待者，只等待进入准入时已经接收的回调（包含正在落盘的一项），后到消息不延长该批等待。准入不占线上请求槽；回调队列满或准入超时时不发送本次日志请求，连接、session 或 boot 改变则使准入失效。该机制用于避免本地持久化积压消耗尚未开始的日志传输预算，不改变上述线上总预算、3 秒无进展时限、8 秒租约或任何报文字段；协议、握手和文档修订仍为 `2.0 / 2.0-design.1 / 2.0-doc.3`。
