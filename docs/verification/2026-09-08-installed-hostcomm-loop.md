...
本地证据见[并发探针与回归产物](evidence/2026-09-08-sqlite-allocator.json)，保留原探针/mutation源码、命令、输出摘要和文件SHA256。慢提交探针基于干净的`9cc47c0d57135ba1ddde2b6a6d9de2a5ebd90d04`；18/45项回归和mutation执行时测试尚未提交，其最终测试blob `583dca29a057b15bfd3e06f872e9b67a4e6174ae`已归档于`abdf18b3f45eb96aafcf35e2792c69d9a2ba7f13`，生产代码未变。所有命令在`backend`目录执行，使用macOS 26.3 arm64、Python 3.13.12、SQLite 3.50.4、SQLAlchemy 2.0.36、aiosqlite 0.20.0；环境是事后对同一未修改解释器的复核，不是原探针内嵌快照。不得将这些本地结论标注为Windows执行。

[修正后的PR #77检查](https://github.com/kevinalliswell/smd-web-hmi/actions/runs/34174233120)实际快照`580f64e1021a50f4cba17f2ef9dd82fc4b372571`七项全部通过，Windows后端767通过/4跳过、覆盖率87.76%，桌面189通过/2跳过，工具86通过。实际安装、17项业务/故障断言、10份报告、清理和最终封装均通过，下载后的报告大小及摘要已逐项核对；安装器摘要`279f1a2148ca7e9343cda2188a5d95e26694b5a6243cf50d2391042fc99fc25a`。该记录仍属于PR构建，最终标签须重新检查。

## TLS 子进程建库与阶段诊断

[文档补充后的检查](https://github.com/kevinalliswell/smd-web-hmi/actions/runs/34176927793)实际PR快照为`526a3b948b05b7cf6deecb6fca71fdb92406bc2f`（分支`8e96af02476bd651bd171143c2b64d145aa2cca8`）。Windows中的生产协调命令与独立分配器测试均通过；随后真实TLS上传中断场景的子进程超过25秒。失败前752项通过、4项平台限定跳过，其余五项基础检查通过，安装包阶段未执行。该日志没有子进程的阶段输出，不能判断具体超时点，或将其表述为已证实的生产通信故障。

[本地TLS夹具证据](evidence/2026-09-08-tls-fixture.json)绑定执行基线`8e96af02476bd651bd171143c2b64d145aa2cca8`、原探针源码/命令和真实SQL事务观察：原41条CREATE均在事务外执行；显式BEGIN后41条均位于同一事务，并以一次事务提交结束。两种夹具在本机都完成了TLS业务与清理，因此不能称为精确复现Windows超时。测试补齐批量建库事务，并增加只输出固定白名单阶段的超时/退出诊断，成功标志移至清理完成后；原25秒总期限和6/10秒连接等待不变。
