...
<template>
  <section class="card control-owner" aria-labelledby="control-owner-title">
    <h2 id="control-owner-title">设备控制权</h2>
    <p role="status">当前控制者：{{ owner === undefined ? '未知' : owner || '尚未领取' }}</p>
    <p class="muted">多个界面共享同一后台。首个写请求自动领取控制权；受控停止仍向其他有操作权限的用户开放。</p>
    <p v-if="error" role="alert" class="error">{{ error }}</p>
    <div class="owner-actions">
      <button :disabled="busy" @click="load">刷新控制者</button>
      <button v-if="owner === null" :disabled="busy" @click="open('claim')">领取控制权</button>
      <button v-if="owner && !isMine && auth.canAdmin" class="danger" :disabled="busy" @click="open('claim')">接管控制权</button>
      <button v-if="isMine" :disabled="busy" @click="open('release')">释放控制权</button>
    </div>
    <ConfirmDialog v-model="confirming" :title="intent === 'release' ? '释放控制权' : takingOver ? '接管控制权' : '领取控制权'" danger :busy="busy" :confirm-disabled="takingOver && !reason.trim()" :close-on-confirm="false" @confirm="confirm">
      ...
      <template v-else-if="takingOver">
        ...
        <textarea id="control-takeover-reason" v-model="reason" maxlength="1000" rows="3" />
      </template>
...
<style scoped>
.control-owner { display: flex; flex-direction: column; gap: 10px; }
h2 { font-size: 14px; }
p, label { font-size: 12px; line-height: 1.6; }
.owner-actions { display: flex; flex-wrap: wrap; gap: 10px; }
.error { color: var(--danger-text); }
textarea { display: block; width: 100%; margin-top: 8px; resize: vertical; }
</style>
