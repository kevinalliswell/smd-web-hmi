...
const versionValid = computed(() =>
  /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(targetVersion.value.trim()),
)
const description = computed(
  () =>
    ({
      idle: '未进入维护',
      prepared: '已准备升级，等待本机安装器',
      claimed: '本机安装器已领取，须在本机完成升级或恢复',
  ...
  >
    ...
    <div class="maintenance-actions">
      ...
      <input
        ...
        placeholder="0.3.0-rc.2"
        :disabled="busy || state?.state !== 'idle'"
      />
      <button
        :disabled="busy"
        @click="run(fetchMaintenance)"
      >
        刷新状态
      </button>
      <button
        class="primary"
        :disabled="busy || !versionValid || state?.state !== 'idle'"
        @click="confirming = true"
      >
        准备升级
      </button>
      <button
        :disabled="busy || state?.state !== 'prepared'"
        @click="run(cancelMaintenance)"
      >
        取消准备
      </button>
    </div>
    <ConfirmDialog
      ...
      @confirm="run(() => prepareMaintenance(targetVersion.trim()))"
    >
      准备升级到
      {{
        targetVersion
      }}。后台将再次检查设备空闲、数据新鲜及实验/操作已完成对账；通过后暂停接收新的控制请求。
    </ConfirmDialog>
  </section>
</template>
