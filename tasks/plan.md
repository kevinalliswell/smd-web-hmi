...
当前候选为 `0.3.0-rc.4`，本次仅交付安装器诊断及缺失维护准备的明确提示，保留rc.3的功能与验收记录。现场先确认覆盖升级缺少维护准备文件；2026-09-07用户确认提权清理rc.2残留服务后安装成功，覆盖升级/空库/通信未验收；旧1.0离线升级限制不变，见[排查记录](../docs/verification/2026-09-06-windows-installer-errors.md)。发布检查和产物须绑定新候选提交，不移动既有rc.3标签。
...
本次固件交接修订为[2.0-doc.2](../docs/hostcomm/v2/firmware-handoff.md)，线上协议/设计号不变。先记录[兼容差异](../docs/hostcomm/v2/compatibility.md)，并行推进Windows安装包到TLS模拟器的现场软件联调和纯C核心；不把模拟器联调设为C核心开发的硬前置。
...
沿用0.3.0候选序列；rc.2 是1.0历史版本，既有 `v0.3.0-rc.3` 标签保持不变，本轮候选为 `0.3.0-rc.4`。共享检查和 Windows 安装 smoke 通过后，将已审查的实现合入主线并创建不可移动的候选标签；标签流水线重新检查和构建，再发布 RC 资产并列出未验证项。PR/tag共用必要检查；版本、包清单、tag与提交一致。每次发布记录数据库、协议、固件、硬件与运行时兼容组合；离线包包含组件清单、hash、来源验证和恢复说明。
...
批准基线 `main@89e5ec1` / `0.3.0-rc.4`；本轮按[ADR-010](../docs/decisions/ADR-010-installed-hostcomm-loop.md)推进下一RC。
