...
沿用0.3.0候选序列；rc.2 是未创建发布标签的历史阶段，本轮候选为 `0.3.0-rc.3`。共享检查和 Windows 安装 smoke 通过后，将已审查的实现合入主线并创建不可移动的候选标签；标签流水线重新检查和构建，再发布 RC 资产并列出未验证项。PR/tag共用必要检查；版本、包清单、tag与提交一致。每次发布记录数据库、协议、固件、硬件与运行时兼容组合；离线包包含组件清单、hash、来源验证和恢复说明。
