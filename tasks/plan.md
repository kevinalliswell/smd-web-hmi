...
本轮开发候选为 `0.3.0-rc.5`，实施租约与通信时序修复、陌生运行恢复和独立Windows联调工具；当前验证范围见[安装版闭环记录](../docs/verification/2026-09-08-installed-hostcomm-loop.md)。上一已发布候选rc.4增加安装器诊断及缺失维护准备提示；2026-09-07用户确认提权清理rc.2残留服务后安装成功，覆盖升级/空库/通信未验收。旧1.0离线升级限制及[现场排查历史](../docs/verification/2026-09-06-windows-installer-errors.md)保留；发布检查和产物须绑定本轮提交，不移动既有标签。
...
本次固件交接修订为[2.0-doc.3](../docs/hostcomm/v2/firmware-handoff.md)，线上协议/设计号不变。[兼容差异](../docs/hostcomm/v2/compatibility.md)记录主机实现与未验边界；独立SmdBench已实现，Windows安装包到TLS模拟器的完整验收仍以同提交执行证据为准。纯C核心可并行推进，不把模拟器联调设为C核心开发的硬前置。
...
沿用0.3.0候选序列；rc.2 是1.0历史版本，rc.3/rc.4等既有标签保持不变，本轮开发候选为 `0.3.0-rc.5`。共享检查、Windows安装smoke和独立SmdBench完整流程通过后，将已审查的实现合入主线并创建不可移动的候选标签；标签流水线重新检查和构建，再发布RC资产并列出未验证项。PR/tag共用必要检查；版本、包清单、tag与提交一致，SmdBench的`acceptance.json`还须匹配实际安装器与工具清单摘要。每次发布记录数据库、协议、固件、硬件与运行时兼容组合；离线包包含组件清单、hash、来源验证和恢复说明。
...
批准基线 `main@89e5ec1` / `0.3.0-rc.4`；本轮按[ADR-010](../docs/decisions/ADR-010-installed-hostcomm-loop.md)推进rc.5。HOST-2006—2008已有软件实现及回归；SmdBench和固定流程已实现，完整Windows验收与RC交付仍在执行，见[本轮验证](../docs/verification/2026-09-08-installed-hostcomm-loop.md)。
...
| P5 | CI与RC交付 | P4与必要质量检查 | 同SHA安装器、独立工具、校验值和未验收清单 |
