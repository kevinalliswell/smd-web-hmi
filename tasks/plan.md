...
M0已合入（PR64）；M1全引用备份/恢复与dev祖先整合（PR48）完成，余分支清理进行中。M1工具链/类型、M2可靠性、M3实验能力与M4交付核心已有软件实现和回归。可选Mock候选契约和真实TCP软件联调已覆盖配方激活、启动和停止冷却归档，但尚未冻结真实固件。

PR65执行共享CI；Windows初轮问题修复后重跑，构建尚待最终证据。M4退出条件中的干净断网安装/服务/实际断电、正式签名以及M5真实设备/24h均未完成。现在的下一步是合入软件验证结果、完成CI与引用清理，随后按外部交付条件推进现场验收；不重开已完成的软件任务，也不提前通过现场门。
